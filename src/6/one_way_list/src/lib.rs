...
impl<T> OnewayList<T> {
    ...
    pub fn push(&mut self, item: T) {
        // 1. 末尾ノードを指すポインタを返す(OnewayList.head or Node.tail)
    ...
    pub fn remove_head(&mut self) {
        match self.head {
            Some(ref mut node) => {
//                let target = std::mem::replace(&mut self.head, Some(self.head.unwrap().next));
                //let first = std::mem::replace(self.head.unwrap().next, None);
                let first = std::mem::replace(&mut self.head, None);
                let first = std::mem::replace(&mut self.head, first.unwrap().next);
//                let first = std::mem::replace(&mut self.head, first.next); // error[E0609]: no field `next` on type `std::option::Option<std::boxed::Box<Node<T>>>`
                /*
                let &mut second = self.head.unwrap().next;
                self.head = second;
                let target = std::mem::replace(&mut self.head, self.head.unwrap().next);
                drop(target);
                */
            },
            _ => (),
...
//        let target = std::mem::replace(self.head, Some(self.head.unwrap().next));
//        drop(target);
//        self.head = self.head.next; // error[E0609]: no field `next` on type `std::option::Option<std::boxed::Box<Node<T>>>`
//        self.head = self.head.unwrap().next; // error[E0507]: cannot move out of borrowed content
//        self.head = self.head.unwrap().next; // error[E0507]: cannot move out of borrowed content
//        std::mem::replace(&mut self.head, self.head.unwrap().next); // error[E0507]: cannot move out of borrowed content
    }
